...
public class ModelBoxFlowTest {
  ...
  public class FlowUnitPassThrough extends FlowUnit {
    public FlowUnitPassThrough() {
      
    }

    @Override
    public Status open(Configuration opts) {
      assertEquals(opts.getString("opt", ""), "value");
      return Status.OK();
    }

    @Override
    public Status process(DataContext data_ctx) {
  ...
  public class FlowUnitBuilderImpl extends FlowUnitBuilder {
    public FlowUnitBuilderImpl() {
      super();
    }
  ...
  public void testFlowRegister() throws Exception {
    ...
    System.out.println(txt);
    Flow flow = new Flow();
    flow.RegisterFlowUnit(new FlowUnitBuilderImpl());
    flow.init("Process", txt);
    flow.startRun();
    FlowStreamIO streamio = flow.CreateStreamIO();
    Buffer data = streamio.createBuffer();
